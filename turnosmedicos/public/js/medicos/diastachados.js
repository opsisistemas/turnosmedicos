	...
	function diasAtencionMedico(){
	    ...
	    $.ajax({
			...
			success:  function (dias)
			{
				...
				//creamos el calendario de edición
				$('#datepicker-center_e').html('<div id="calpicker_e"></div>');

				configurarCalendarios(dias);
			}
		});
	}

	function configurarCalendarios(dias){
		...
		$('#calpicker_e').datepicker({
			...
			$('#fecha_dp_e').val(
				$('#calpicker_e').datepicker('getFormattedDate')
			);
			calendarClick();
		});
	}
...
$('#btn-cambiar').on('click', function(){
	//habilitamos el calendario de edición
	$("#calendar-picker_e").attr('class', '')
});